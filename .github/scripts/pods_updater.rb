...
def collect_deprecations_report(adapter_names: [])
  ...
  sh_allow_fail(<<~'BASH')
    ...
    # From our tee'd xcodebuild output
    if [ -f build/reports/pods_tests.log ]; then
      grep -ai 'deprecated' build/reports/pods_tests.log >> "$RAW" || true
    fi

    # From xcactivitylogs in workspace DerivedData and user's DerivedData (if any)
    { \
      find "${GITHUB_WORKSPACE:-$PWD}/DerivedData" -type f \( -path "*/Logs/Build/*.xcactivitylog" -o -path "*/Logs/Test/*.xcactivitylog" \) -print0 2>/dev/null; \
      find ~/Library/Developer/Xcode/DerivedData -type f \( -path "*/Logs/Build/*.xcactivitylog" -o -path "*/Logs/Test/*.xcactivitylog" \) -print0 2>/dev/null; \
    } | while IFS= read -r -d '' ACT; do
          (gzcat "$ACT" 2>/dev/null || cat "$ACT") | strings | grep -ai 'deprecated' || true
        done >> "$RAW"

    # Also parse xcresult bundles for 'deprecated'
    if [ -d Result ]; then
    ...
    # Keep only compiler warning lines that look like Xcode output:
    #   <file>.<ext>:<line>:<col>: warning: ... is deprecated ...
    DEPR="build/reports/deprecations/deprecations.txt"
    : > "$DEPR"
    grep -aiE '\.(swift|m|mm|h|hpp|cpp):[0-9]+:[0-9]+: warning: .*is deprecated' "$RAW" \
      | sort -u > "$DEPR" || true

    COUNT=$(wc -l < "$DEPR" 2>/dev/null || echo 0)

    # Best-effort adapter detection from repo + warning lines
    ...
    if [ -s "$DEPR" ] && [ -s build/reports/deprecations/all_adapters.txt ]; then
      while IFS= read -r ADP; do
        if grep -aiqE "(Adapters/|/)${ADP}(/|\\.| |:|$)|(^|[^A-Za-z0-9_])${ADP}([^A-Za-z0-9_]|$)" "$DEPR"; then
    ...
    {
      echo "## Deprecated API warnings";
      echo "";
      echo "- Findings: ${COUNT:-0}";
      echo "";
      if [ "${COUNT:-0}" -gt 0 ]; then
        if [ -s build/reports/deprecations/adapters.txt ]; then
          echo "### Affected adapters (detected)";
          sort -u build/reports/deprecations/adapters.txt | sed 's/^/- /'
          echo ""
        fi
        echo "<details><summary>Warnings (first 200)</summary>";
        echo "";
        echo "\`\`\`text";
        head -n 200 "$DEPR" || true
        echo "\`\`\`";
        echo "";
        echo "</details>";
      else
        echo "No deprecated API usage found in build/test logs.";
      fi
      echo "";
      echo "_Generated by Pods Updater workflow._"
