//
//  TaurusXDirectAdViewDemandProvider.swift
//  BidonAdapterTaurusX
//
...
final class TaurusXBannerDemandAd: NSObject, DemandAd {
    private let bannerAd: TaurusXBanner
    
    var id: String {
        return String(hash)
    }

    init(_ bannerAd: TaurusXBanner) {
        self.bannerAd = bannerAd
    }
}

final class TaurusXDirectAdViewDemandProvider: NSObject, DirectDemandProvider {
...
extension TaurusXDirectAdViewDemandProvider: AdViewDemandProvider {
...
extension TaurusXDirectAdViewDemandProvider: TaurusXBannerDelegate {
    func adLoadFinish(_ bannerView: UIView) {
        guard let bannerAd else {
            response?(.failure(.noFill("Missing placement ID")))
            response = nil
            return
        }
...
extension UIView: @retroactive AdViewContainer {
