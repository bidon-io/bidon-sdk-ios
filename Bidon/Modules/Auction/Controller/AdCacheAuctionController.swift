...
final class AdCacheAuctionController<AdTypeContextType: AdTypeContext>: AuctionController {
    ...
    func start() {
        _pendingOperations.forEach { op in
            let finishDemandOperation = createFinishDemandOperation(op)
            finishDemandOperation.addDependency(op)

            queue.addOperation(op)
            queue.addOperation(finishDemandOperation)
            finishAuctionOperation!.addDependency(op)
        }
    ...
    private func operation<T: AuctionOperation>(adUnit: AdUnitModel? = nil, build: ((T.BuilderType) -> ())? = nil) -> T
    where T.BuilderType.AdTypeContextType == AdTypeContextType {
        return T { builder in
            ...
            builder.withExecutionCondition { [weak self] in
                ...
                if let shouldCancel = self.shouldCancelBeforeNextAdUnit,
                   shouldCancel(adUnit.pricefloor) {
                    return false
                }
                return true
            }
